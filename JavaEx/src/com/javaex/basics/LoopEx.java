...
private static void breakEx() {
	...
	//어디까지 루프를 돌려야 할지 판단이 어려움으로 -> while문으로 
	int num=1;
	while (true) {
...
private static void  continueEx() {
	//1~20수 중 2의 배수, 3의 배수를 제외한 나머지를 출력 
		for (int i=1;i<=20;i++) {
			if(i%2==0 || i%3==0 ) continue;
			//2의 배수, 3의 배수면 아래 코드가 진행되지 않고 다음 루프를 수행함 
...
private static void doWhileEx() {
	...
	private static void whileInfinity() {
		//while 문은 반복조건을 개발 세팅해야함
		//-> 무한 루프에 빠질 위험
		// 경우에 따라 의도적으로 무한 루프를 만들기도 함 
		//반복 조건이 항상 true 여야만 한다
		while (true) {
